...
(:action continue-cut-stop-4
	:parameters (?x - object
		?y - object)
	:precondition (and
		(not (inverse-splice-next-copy ?y))
		(continue-cut-lock ?x ?y))
	:effect (and
		(invalid )))
...
(:action begin-inverse-splice-special-case-stop-3
	:parameters (?x - object)
	:precondition (and
		(s-last-copy ?x)
		(begin-inverse-splice-special-case-lock ?x))
...
(:action invert-single-gene-a-copy
	...
	:effect (and
		...
		(not (normal-copy ?x))
		(inverted-copy ?x)
		(not (splice-point-2-copy ?x))(increase (total-cost ) 1)))
